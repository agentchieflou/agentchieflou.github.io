...
def _gemini_profile(resume_text, repo_summaries):
    prompt = (
        ...
        "repo activity), not resume keywords alone.\n"
        "target_titles = 4-6 job titles this person is genuinely competitive for TODAY, "
        "ordered by fit. Their stated direction is business analytics and process "
        "optimization with a technical focus — building tools that optimize how teams "
        "work — extending into technically-focused strategy and AI-leaning roles. Weight "
        "titles that way. They are roughly 4 years into their career, so titles must be a "
        "realistic step up (Senior / Lead / Manager at most), never Principal, Staff, "
        "Director or Head of. They only want fully-remote, full-time roles.\n"
        "Exclude legacy skills the person no longer uses: SAS, Stata, NLP/BERT, COBOL, Go "
        "(do not emit them even if something in the source material mentions them). "
